...
<body>
	...
	<header>
       ...
       <c:if test="${not empty mb.id}">
       	<li><a class = "btn btn-success" style="width:200px; height:50px;font-size:25px;"href = "/shopping/cart?id=${mb.id}">장바구니</a></li>
       	<li><a class = "btn btn-success" style="width:200px;height:50px;font-size:25px;"href = "/shopping/buy/detail">구매내역</a></li><br><br>
       </c:if>
	<!-- 리스트 반복문  -->
	<div class="row">
			...
			<c:if test = "${shopping.ea > 0 || mb.type == 'admin'}">
				<div class="box" >
					<form action="/shopping/cart?item=${shopping.item}" method="post" class="post">
						<div class="card">
								<div class="box1">
									<c:choose>
						...
						<div class="card-body">
							...
							<div>가격:<fmt:formatNumber value="${shopping.price}"
									pattern="###,###,###" var="formattedPrice" /></div>
						...
						<c:if test = "${mb.type == 'admin'}">
							<form method="get" action="/shopping/update">
								<input type="hidden" value="${shopping.item}" name="item">
								<button id="delete"class="btn btn-primary"style="margin-top:10px;">수정</button>
							</form>
						...
						<tr align="center">
							<td colspan="2">
								...
								<form method="post" action="/shopping/addcart">
									...
									</select>개 <input type="submit" class="btn btn-primary"id="checkcart" value="장바구니에 담기">
								</form>
							</td>
						</tr>
						</c:if>
			...
			<c:if test = "${mb.type == 'admin'}">
			<form action = "/shopping/write" method = "get">
