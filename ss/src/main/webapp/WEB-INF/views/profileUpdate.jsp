...
<body>
	...
	<form action = "/member/profile/update" method = "post" enctype="multipart/form-data">
		...
		<input type = "text" name = "animal" id = "animal" value = "${profile.animal}" placeholder = "동물종류"><br>
		<input type = "text" name = "p_name" id = "p_name" value = "${profile.p_name}" placeholder = "동물이름"><br>
		<select name="p_gender" id="p_gender">
		  <option value="수컷" ${profile.p_gender == '수컷' ? 'selected' : ''}>수컷</option>
		  <option value="암컷" ${profile.p_gender == '암컷' ? 'selected' : ''}>암컷</option>
		  <option value="중성" ${profile.p_gender == '중성' ? 'selected' : ''}>중성</option>
		</select><br>
